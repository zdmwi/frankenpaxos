...
class RT3MultiPaxosNet:
    ...
    def placement(self) -> Placement:
        ...
        return self.Placement(
            ...
            monitors=portify(
                cycle_take_n(self._input.num_monitors,
                             self._cluster['monitors'])),
    ...
    def config(self) -> proto_util.Message:
        return {
            ...
            'monitor_address': [{
                'host': e.host.ip(),
                'port': e.port
            } for e in self.placement().monitors],
        }


# Suite ########################################################################
class RT3MultiPaxosSuite(benchmark.Suite[Input, Output]):
    ...
    def run_benchmark(self, bench: benchmark.BenchmarkDirectory,
        ...
        # Write config file.
        net = RT3MultiPaxosNet(self._cluster, input)
        config = net.config()
        ...
        for (i, acceptor) in enumerate(net.placement().acceptors):
            p = bench.popen(
                ...
                cmd=java(input.acceptor_jvm_heap_size) + [
                    ...
                    'frankenpaxos.rt3multipaxos.AcceptorMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
        ...
        for (i, matchmaker) in enumerate(net.placement().matchmakers):
            p = bench.popen(
                ...
                cmd=java(input.matchmaker_jvm_heap_size) + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.rt3multipaxos.MatchmakerMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
        ...
        for (i, reconfigurer) in enumerate(net.placement().reconfigurers):
            p = bench.popen(
                ...
                cmd=java(input.reconfigurer_jvm_heap_size) + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.rt3multipaxos.ReconfigurerMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
        ...
        for (i, replica) in enumerate(net.placement().replicas):
            p = bench.popen(
                ...
                cmd=java(input.replica_jvm_heap_size) + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.rt3multipaxos.ReplicaMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
        ...
        for (i, leader) in enumerate(net.placement().leaders):
            p = bench.popen(
                ...
                cmd=java(input.leader_jvm_heap_size) + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.rt3multipaxos.LeaderMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
        ...
        # Launch monitors.
        monitor_procs: List[proc.Proc] = []
        for (i, monitor) in enumerate(net.placement().monitors):
            p = bench.popen(
                host=monitor.host,
                label=f'monitor_{i}',
                cmd=java(input.monitor_jvm_heap_size) + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.rt3multipaxos.MonitorMain',
                    '--index',
                    str(i),
                    '--config',
                    config_filename,
                    '--log_level',
                    input.monitor_log_level,
                    '--prometheus_host',
                    monitor.host.ip(),
                    '--prometheus_port',
                    str(monitor.port + 1) if input.monitored else '-1',
                ],
            )
            if input.profiled:
                p = perf_util.JavaPerfProc(bench, monitor.host, p,
                                           f'monitor_{i}')
        ...
        for (i, client) in enumerate(net.placement().clients):
            p = bench.popen(
                ...
                cmd=java(input.client_jvm_heap_size) + [
                    '-cp',
                    os.path.abspath(args['jar']),
                    'frankenpaxos.rt3multipaxos.ClientMain',
                    '--host',
                    client.host.ip(),
                    '--port',
                    str(client.port),
        ...
        driver_proc: proc.Proc = bench.popen(
            ...
            cmd=java(input.driver_jvm_heap_size) + [
                '-cp',
                os.path.abspath(args['jar']),
                'frankenpaxos.rt3multipaxos.DriverMain',
                '--host',
        ...
        for p in (leader_procs + matchmaker_procs + reconfigurer_procs +
                  acceptor_procs + replica_procs + monitor_procs + [driver_proc]):
            p.kill()
